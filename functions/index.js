...
// 'defineSecret'을 직접 import하는 대신, 'functions' 객체를 통해 접근하도록 수정합니다.
// import { defineSecret } from "firebase-functions/params"; // 이 줄을 삭제합니다.

const GEMINI_API_KEY = functions.params.defineSecret("GEMINI_API_KEY"); // 이렇게 수정합니다.
...
`{
  ...
  try{
    ...
    console.error("Model response is still not a valid JSON after extraction:", rawJson);
    throw new functions.https.HttpsError("invalid-argument", "모델 응답 JSON 파싱 실패", { rawResponse: rawJson });
  }
