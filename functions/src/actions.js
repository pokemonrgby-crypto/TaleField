...
export async function playCard(data, context) {
    ...
    return await db.runTransaction(async (tx) => {
        ...
        if (!card) throw new HttpsError("not-found", "해당 카드가 손에 없습니다.");
        ...
        const newOps = card.dsl.map(op => ({
            ...op,
            casterUid: uid,
            targetUid: targetUid || (op.defaultTarget === 'self' ? uid : null), // 기본 타겟 설정
            cardName: card.name,
        }));
        ...
        tx.update(matchRef, {
            [`players.${uid}`]: player,
            stack: newStack,
            phase: 'reaction', // 반응 창 열기
            reactionEndsAt: FieldValue.serverTimestamp() // 실제로는 (현재시간 + 7초)
        });
...
// ... startGame, react, endTurn 등의 다른 action 함수들도 여기에 위치하게 됩니다 ...
